class AddQrcodeToImageJob < ApplicationJob
  ...
  def generate_qrcode_image(qrcode_url)
    qrcode_image = QrcodeImage.create(image_id:@image.id,member_id:@image.member_id,product_id:@image.prodcut_id)
    filename = Digest::MD5.hexdigest(@qrcode_name)+@qrcode_ext
    store_dir = "/uploads/qrcode_image/#{qrcode_image.id}/"
    ActiveRecord::Base.connection.execute("update qrcode_images set image_path='#{filename}' where id = #{qrcode_image.id}")

    CombineImage.new(@image.path.url,qrcode_url,outfile: store_dir+filename)
  end
end
